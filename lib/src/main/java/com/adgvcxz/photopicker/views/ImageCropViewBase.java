...
import com.adgvcxz.photopicker.util.BitmapLoadUtils;
...
public abstract class ImageCropViewBase extends ImageView {
    ...
    public Bitmap getCroppedImageMaxSize(int maxWidth, int maxHeight) {
        ...
        if (imageFilePath != null && (size[0] < maxWidth || size[1] < maxHeight)) {
            ...
            sourceBitmap = BitmapLoadUtils.decode(imageFilePath, imageWidth, imageHeight);
            scale = scale * ((float) viewBitmap.getWidth() / (float) sourceBitmap.getWidth());
            size = new int[]{sourceBitmap.getWidth(), sourceBitmap.getHeight()};
        }
        ...
        if (y + actualCropHeight > size[1]) {
            actualCropHeight = size[1] - y;
        }

        if (x + actualCropWidth > size[0]) {
